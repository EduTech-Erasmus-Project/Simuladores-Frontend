...
export class ListaExpertosRegistradosPlaformaComponent implements OnInit {
  ...
  private async loadParticipantes() {
    ...
    try {
      const expertoAprobado = await this.expertoService
  ...
  public async bloquarCuenta(id) {
    this.estado = true;
    await this.expertoService.bloqueoCuentaEvaluador(id).subscribe(
      (result) => {
        this.expertoService.emitEvent(true);
        Swal.fire({
          icon: "success",
          title: `La cuenta del evaluador a sido ${
            result.estado ? "activada" : "desactivada"
          } `,
