...
@Component({
  ...
  styleUrls: [
    "./presentacion-inicio-experto.component.css",
...
export class PresentacionInicioExpertoComponent implements OnInit {
  ...
  async ngOnInit() {
    this.servicioSeleccionarEjercitario
      .obtenerEjercitarios()
      .then((ejercitariosRep) => {
        this.ejercitarios = ejercitariosRep;
      });

    this.servicioSeleccionarEjercitario
      .recuperarInformacionDeEscenario(1)
      .then((escenario) => {
  ...
  seleccionEscenario(): void {
    this.servicioSeleccionarEjercitario
      .recuperarInformacionDeEscenario(this.selectedEjercitario.value)
      .then((escenario) => {
        this.escenario = escenario;
      });
  }
  ...
  listarLabelsTipoDeGenero() {
    this.servicioConsultasLabelsGrafica
      .recuperarListaDeGenero(this.correoResponsableDatos)
      .subscribe((genero) => {
  ...
  crearGraficaInicioExperto() {
    ...
    this.generos.forEach((genero) => {
      this.servicioSeleccionarEjercitario
        .crearGraficaPaginaInicio(this.correoResponsableDatos)
        .subscribe((datosParaGrafica) => {
          datosParaGrafica.participantes.forEach((informacionParticipante) => {
            if (genero == informacionParticipante.participanteGenero) {
              if ("Fisica" == informacionParticipante.tipoDiscapacidad) {
                discapacidadFisica =
                  discapacidadFisica +
                  informacionParticipante.calificaciones[0].calificacion;
                discapacidadFisicaTiempo =
                  discapacidadFisicaTiempo +
                  informacionParticipante.calificaciones[0].tiempo;
                console.log(
                  "discapacidadFisicaTiempo: " + discapacidadFisicaTiempo
                );
              }
