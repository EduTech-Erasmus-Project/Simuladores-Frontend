...
@Component({
  ...
  styleUrls: ['./register-experto.component.css']
})
export class RegisterExpertoComponent implements OnInit {
  ...
  constructor(private autentificacionService: AutentificacionUsuarioService, private informacionExperto: InformacionEvaluadorService) { }
  ...
  validadCorreo() {
    ...
    this.autentificacionService.checkEmail(this.emailExperto).subscribe(res => {
  ...
  crearNuevoExperto(){
    ...
    this.informacionExperto.registrarNuevoExperto(this.experto).subscribe(res=>{
